...
static int f2fs_file_open(struct inode *inode, struct file *filp)
{
	...
	// 	struct f2fs_delayed_truncate *dt;
		
	// 	dt = kzalloc(sizeof(*dt), GFP_KERNEL);
	// 	if (!dt)
	// 		return -ENOMEM;
			
	// 	dt->original_size = inode->i_size;
	// 	dt->truncate_pending = true;
	// 	filp->private_data = dt;
	// 	filp->f_flags &= ~O_TRUNC;  // 临时清除标志
	// 	pr_info("[f2fs_open] Saved ori_size %lld delayed truncate\n", dt->original_size);
	// }
...
static int f2fs_release_file(struct inode *inode, struct file *filp)
{
	 ...
	 */
	pr_info("[f2fs_release] inode=%p, ino=%lu, i_size=%lld\n",
		inode, inode->i_ino, inode->i_size);
...
static int f2fs_create_snapshot(struct file *filp, unsigned long arg)
{
    ...
    struct inode *src_inode = NULL, *parent_inode = NULL, *new_inode = NULL;
	// 被快照目录和生成的快照目录的inode，以及生成的快照目录的inode
	...
	path1 = memdup_user(user_paths[0], len);//PATH_MAX
	if (IS_ERR(path1)){
	...
	path2 = memdup_user(user_paths[1], len);
	if (IS_ERR(path2)) {
		err = PTR_ERR(path2);
		kfree(path1);
		return err;
	...
	e = __loup_nat_cache(nm_i, src_inode->i_ino);
	if(nat_get_version(e) == 0){
		pr_info("f2fs snapshot: %s can't create snapshot\n",path1);
		return -EINVAL;
	}
	...
	/* ---------- 分离 path2 的父目录与文件名 ---------- */
	{
		...
		if (!slash) {
			parent = kstrdup(".", GFP_KERNEL);
			name = kstrdup(path2, GFP_KERNEL);
		} else if (slash == path2) {
			parent = kstrdup("/", GFP_KERNEL);
			name = kstrdup(slash + 1, GFP_KERNEL);
		} else {
			parent = kmalloc(slash - path2 + 1, GFP_KERNEL);
			if (!parent) { err = -ENOMEM; goto out_put_src; }
			memcpy(parent, path2, slash - path2);
			parent[slash - path2] = '\0';
			name = kstrdup(slash + 1, GFP_KERNEL);
		}
		if (!parent || !name) { err = -ENOMEM; goto out_put_src; }
	}
	/* ---------- 检查 path2 的父目录是否存在 ---------- */
    ...
    /* ---------- 创建 path2 目录 ---------- */
	{
		...
		// 检查是否是inline dir
		entry_cnt = NR_INLINE_DENTRY(src_inode);
		bitmap_size = INLINE_DENTRY_BITMAP_SIZE(src_inode);
		reserved_size = INLINE_RESERVED_SIZE(src_inode);
	...
	if (f2fs_has_inline_dentry(src_inode)) {
        pr_info("[mk_snap] Dir(%lu) uses inline dentry\n", src_inode->i_ino);
		//获取该node的page
		...
		pr_info("[mk_snap] Dir(%lu) uses non-inline dentry\n", src_inode->i_ino);
	...
	delta_ns = end - start;
	pr_info("[mk_snap]: mk snap time: %lld ns, src:dst[%lu,%lu]\n", delta_ns,src_inode->i_ino,new_inode->i_ino);
	...
	pr_info("[mk_snap] magic page flag has been set by snap_ioctl~!\n");
...
static ssize_t f2fs_file_read_iter(struct kiocb *iocb, struct iov_iter *iter)
{
	...
	if(f2fs_has_inline_data(inode)){
		pr_info("[ read ] has_inline_data=%d, i_size=%lld\n",
		...
		ipage = f2fs_get_node_page(sbi, inode->i_ino);
		page_addr = inline_data_addr(inode, ipage);
		src_fi = F2FS_INODE(ipage);
		f2fs_put_page(ipage, 1);
		dentry = d_find_any_alias(inode);

		pr_info("read file [%lu,%s],f2fs_inode_size[%lu]\n",
			inode->i_ino, dentry->d_name.name, src_fi->i_size); 
		// if (f2fs_has_inline_data(inode)){
...
static ssize_t f2fs_file_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	// struct f2fs_delayed_truncate *dt = file->private_data;
	
	// if (dt && dt->truncate_pending) {
	// 	pr_info("[write_iter] Delayed truncate: original_size=%lld\n",
	// 		dt->original_size);
		
	// 	// 执行你的自定义逻辑
	// 	pr_info("[write_iter] Processing before truncate...\n");
	// 	struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
	// 	struct page *ipage;
	// 	ipage = f2fs_get_node_page(sbi, inode->i_ino);
	...
	// 	pr_info("[ write start ] has_inline_data=%d, i_size=%lld\n",
	// 		f2fs_has_inline_data(inode),
	// 		inode->i_size);
	// 	// 执行截断
	// 	truncate_setsize(file_inode(file), 0);
	// 	dt->truncate_pending = false;
	// 	// 清理
	// 	kfree(dt);
	// 	file->private_data = NULL;
	// 	pr_info("[ write start truncate ] has_inline_data=%d, i_size=%lld\n",
	// 		f2fs_has_inline_data(inode),
	// 		inode->i_size);
	// }
	...
	struct f2fs_inode *src_fi;
	// struct inode *snap_inode, *new_inode;
	void *page_addr;
	struct page *ipage;
	size_t idx;
	dentry = d_find_any_alias(inode);
	pr_info("write file [%lu,%s]\n",inode->i_ino,dentry->d_name.name); 
	// smp_rmb();
	ipage = f2fs_get_node_page(sbi, inode->i_ino);
	if (f2fs_has_inline_data(inode)){
		page_addr = inline_data_addr(inode, ipage);
...
start_snap:
    ...
    while (!snap_isEmpty(&stack)) {`
		...
		struct page *pra_ipage, *snap_ipage, *page;
		...
		if (S_ISDIR(pra_inode->i_mode)) {
			pr_info("COW (%lu) start create new\n", pra_inode->i_ino);
			// 删除指定目录项
			// dentry ?
			pr_info("COW gen:snap[%lu,%s],copy:son[%lu,%s]\n",snap_inode->i_ino, 
				d_find_any_alias(snap_inode)->d_name.name, son_inode->i_ino, dentry->d_name.name);
			de = f2fs_find_entry(snap_inode, &dentry->d_name, &page);
			if(!de){
				pr_info("not found [%s]dentry\n",dentry->d_name.name);
				goto normal;
			}
			f2fs_delete_entry(de, page, snap_inode, son_inode);
			
			// 判断删除是否成功
			de = f2fs_find_entry(snap_inode, &dentry->d_name, &page);
			if(!de){
				pr_info("[%s]dentry del success!!!\n",dentry->d_name.name);
			}else{
				pr_info("[%s]dentry del failed!!!\n",dentry->d_name.name);
			}
			// 查询dentry是否存在
			tmp_dentry = lookup_one_len(dentry->d_name.name, d_find_any_alias(snap_inode), strlen(dentry->d_name.name));
			// tmp_dentry = lookup_one_len("fx", d_find_any_alias(snap_inode), strlen(dentry->d_name.name));
			if (IS_ERR(tmp_dentry)) {
				ret = PTR_ERR(tmp_dentry);
				tmp_dentry = NULL;
				pr_info("lookup_one_len failed!!!\n");
			}
			// 添加目录项
			// 在snap_inode下创建一个新的inode，mode保持和被快照目录下的inode一致
			mode = pra_inode->i_mode;
			// ret = f2fs_mkdir(NULL, pra_inode, dentry, mode);
			if(S_ISDIR(son_inode->i_mode)){
				ret = vfs_mkdir(mnt_user_ns(parent_path.mnt), pra_inode, tmp_dentry, mode);
			}else if(S_ISREG(son_inode->i_mode)){
				pr_info("create new file\n");
				ret = vfs_create(mnt_user_ns(parent_path.mnt), pra_inode, tmp_dentry, mode, true);
			}

			new_inode = tmp_dentry->d_inode;
			if(!new_inode){
				pr_info("new_inode failed!!!\n");
				goto normal;
			}
			dentry = d_find_any_alias(new_inode); 
			// pr_info("cow tp3 new inode[%lu],name[%s]\n",new_inode->i_ino,dentry->d_name.name);
			// dentry = d_find_any_alias(son_inode); 
			// pr_info("cow tp3 copy inode[%lu],name[%s]\n",son_inode->i_ino,dentry->d_name.name);

			// 处理子文件
			if (f2fs_has_inline_data(son_inode) && S_ISREG(son_inode->i_mode)){
				pr_info("son(new) is file with inline data\n");
				set_inode_flag(new_inode, FI_INLINE_DATA);

				tipage = f2fs_get_node_page(sbi, son_inode->i_ino);
				src_fi = F2FS_INODE(tipage);

				nipage = f2fs_get_node_page(sbi, new_inode->i_ino);
				inline_dentry = inline_data_addr(son_inode, tipage);
				inline_dentry2 = inline_data_addr(new_inode, nipage);
			
				f2fs_truncate_inline_inode(new_inode, nipage, 0);
				...
				// make_dentry_ptr_inline(new_inode, &d, inline_dentry2);
			} else if(!f2fs_has_inline_dentry(pra_inode) && S_ISREG(pra_inode->i_mode)) {
				pr_info("son(new) is file without inline data\n");
				...
				for (idx = 0; idx < DEF_ADDRS_PER_INODE; idx++) {
					if (src_fi->i_addr[idx] != NULL_ADDR && src_fi->i_addr[idx] != NEW_ADDR) {
						...
						if(src_fi->i_addr[idx] != NEW_ADDR){
							...
							// if(idx == 0){
							// 	// 实际命令应该是new_dpage
							// 	page = f2fs_get_lock_data_page(new_inode, idx, false);
			...
			// 处理子目录
			if (f2fs_has_inline_data(son_inode) && S_ISDIR(son_inode->i_mode)){
				pr_info("son(new) is dir with inline data\n");
				set_inode_flag(new_inode, FI_INLINE_DATA);
				...
				f2fs_put_page(tipage, 1);
				// make_dentry_ptr_inline(new_inode, &d, inline_dentry2);
			} else if(!f2fs_has_inline_dentry(pra_inode) && S_ISDIR(pra_inode->i_mode)) {
				pr_info("son(new) is dir without inline data\n");
...
out:
	...
	// // test
	// dentry = d_find_any_alias(inode);
	// pr_info("[after] write file [%lu,%s]\n",inode->i_ino,dentry->d_name.name); 
	// // smp_rmb();
	// ipage = f2fs_get_node_page(sbi, inode->i_ino);
	// if (f2fs_has_inline_data(inode)){
	// 	page_addr = inline_data_addr(inode, ipage);
	// 	pr_info("[after]  inline data addr, %x\n",page_addr); 
	// 	for (idx = 0; idx < 10; idx++) {
	// 		pr_info("[after]  %02x\n", ((unsigned char *)page_addr)[idx]);
	// 	}
	// 	// unsigned char *data = (unsigned char *)page_addr;
	...
	// }
	pr_info("[ write end ] has_inline_data=%d, i_size=%lld\n",
		f2fs_has_inline_data(inode),
		inode->i_size);
	return ret;
}
