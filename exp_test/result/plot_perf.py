...
# 设置学术风格 - 确保所有字体都是 Times New Roman
mpl.rcParams.update({
    'font.family': 'serif',
...
for i, (fs, data) in enumerate(rand_read_norm.items()):
    offset = (i - 1) * width
    ax1.bar(x_rand + offset, data, width, label=fs, color=colors[fs], alpha=0.85, edgecolor='gray', linewidth=0.5)
...
for i, (fs, data) in enumerate(rand_write_norm.items()):
    offset = (i - 1) * width
    ax2.bar(x_rand + offset, data, width, label=fs, color=colors[fs], alpha=0.85, edgecolor='gray', linewidth=0.5)
...
for i, (fs, data) in enumerate(seq_read_norm.items()):
    offset = (i - 1) * width
    ax3.bar(x_seq + offset, data, width, label=fs, color=colors[fs], alpha=0.85, edgecolor='gray', linewidth=0.5)
...
for i, (fs, data) in enumerate(seq_write_norm.items()):
    offset = (i - 1) * width
    ax4.bar(x_seq + offset, data, width, label=fs, color=colors[fs], alpha=0.85, edgecolor='gray', linewidth=0.5)
...
# 创建公用图例在顶部
handles = [plt.Rectangle((0,0),1,1, color=colors['f2fs'], alpha=0.85),
           plt.Rectangle((0,0),1,1, color=colors['snapfs'], alpha=0.85),
           plt.Rectangle((0,0),1,1, color=colors['snapfs-snap'], alpha=0.85)]
fig.legend(handles, legend_labels, loc='upper center', bbox_to_anchor=(0.5, 0.98), ncol=3, prop={'family': 'Times New Roman'})
